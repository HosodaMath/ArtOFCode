...
const setup = () => {
    ...
    for (let count = 0; count < 20; count++) {
        /// ブラシの座標設定
        location1[count] = new Vector2(Mathematics.random(width / 4, width - width / 4 - SHIFT_X1), Mathematics.random(height / 4, height - height / 4 - SHIFT_y1));
        velocity1[count] = new Vector2(Mathematics.random(-2, 2), Mathematics.random(-2, 2));
        const SHIFT_X2 = 20;
        const SHIFT_y2 = SHIFT_X2;
        location2[count] = new Vector2(Mathematics.random(width / 4 + SHIFT_X2, width - width / 4 - SHIFT_X2), Mathematics.random(height / 4 + SHIFT_y2, height - height / 4 - SHIFT_y2));
