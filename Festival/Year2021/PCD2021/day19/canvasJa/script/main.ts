...
let moon1: Moon;
let moon2: Moon;
let moon_location1: Vector2 = new Vector2(0, 0);
let moon_radius1: number = 0;
let moon_location2: Vector2 = new Vector2(0, 0);
let moon_radius2: number = 0;

// 山の世界の変数
let mountain: Mountain1[] = [];
let mountain_color: CanvasGradient[] = [];
...
const setup = () => {
  ...
  // mountain field
  let mountain_data1: Vector2[] = []
  let mountain_data2: Vector2[] = []
  let init_data_x1: number[] = [0, width / 2.0, width];
  let init_data_y1: number[] = [height, height / 2.0, height];
  let init_data_x2: number[] = [0, width / 4.0, width / 2.0];
  let init_data_y2: number[] = [height, height - height / 4.0, height];
 ...
 */
const setStar = (star_location: Vector2[], star_radius: number[]) => {
 ...
 */
const setMoon = (moon_location1: Vector2, moon_radius1: number, moon_location2: Vector2, moon_radius2: number) => {
 ...
 */
const setMountain = (mountain_data1: Vector2[], mountain_data2: Vector2[]) => {
...
const renderTitle = () => {
  ...
  gl.fillText("こんにちは、あおいおつきさん", width / 2, height / 4);
}
...
const renderAuthor = () => {
  ...
  gl.fillStyle = "#ffffff"
  //gl.fillText("Hello Blue Moon", width / 2, height / 4);
  gl.fillText("さく・え　ほそだ しんご", width / 2,height - height / 10);
}
