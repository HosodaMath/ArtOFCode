/**
 * 今回は音源サンプルは効果音としてのみ使用としています。
 */
import { starParticleSketch } from "./sketch/starParticleSketch";
import {
  createStartWindow,
  createFullScreen,
  createAudioController,
} from "./window/window";
import WindSound1 from "./assets/sound/wind1.mp3";
import "sanitize.css";
import "./style.css";
...
startSystem.startButton.addEventListener("click", () => {
  ...
  const loadSample = async (audioCtx: AudioContext): Promise<AudioBuffer> => {
    const response = await fetch(WindSound1);
