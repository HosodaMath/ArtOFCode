 ...
 * 中心から一定間隔で出力する。
 * @todo 軌道に乱数を加える -> x
 * @todo 軌道にNoiseを加える -> 中断
 * @todo 軌道に三角関数を加える -> x
 * @todo 軌道に重力を加える -> 中断
 * @todo Object2D(円、ひし形など)を大きくする
 */
...
const sketch = (p: P5) => {
  ...
  let particle: Particle.RhombusParticle[] = [];
  /**
   * Particle生成の準備
   */
  const createParticle = () => {
    [...Array(MAX).keys()].forEach((_count) => {
      ...
      const initPositionY = p.height / 4.0;
      ...
      // Patricleの大きさの初期化
      const initRadius = p.random(50, 100);
      const radius = initRadius;

      const ci = new Particle.RhombusParticle(
        p,
        position,
        velocity,
        acceleration,
