...
export class Move {
   ...
   * @param image
   */
  render = (shader: P5.Shader, image: P5.Image, canvas: P5.Graphics) => {
    canvas.background(100, 200, 200);
    canvas.shader(shader);
    shader.setUniform("uTexture", image);
    shader.setUniform("uFrameCount", this.p.frameCount * 0.5);
    canvas.push();
    canvas.translate(this.location1.x, this.location1.y, this.location1.z);
    canvas.rotateX(0.5 * this.anim);
    canvas.rotateY(this.anim);
