/*
  Processingの座標データを渡すただしそのままでは渡せないので注意が必要
  1. floatに変換する必要がある
  2. vec2 -> vec4に変換する必要があるあるいはvec4 -> vec2に変換する必要がある
*/
...
void main(){
  vec4 baseCoordinate = vertTexCoord;
  vec2 coord = baseCoordinate.xy;
  coord = (2.0 * gl_FragCoord.xy - resolution) / min(resolution.x, resolution.y);
  
  float radian = radians(time * 3.15);

  vec3 background_color = mix(
    vec3(0.9804, 0.6314, 0.8235), 
    vec3(0.9725, 0.9804, 0.6314), 
    coord.y
  );
  
  float scale = cos(0.3 * time) + 5.0;
