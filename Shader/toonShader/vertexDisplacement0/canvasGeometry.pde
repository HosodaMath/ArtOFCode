void canvasPlane(PGraphics canvas, float width, float height){
  float w = width / 2.0;
  float h = height / 2.0;
  canvas.beginShape();
  canvas.vertex(-w, h, 0);
...
void canvasCircle(PGraphics canvas, float segment, float radius){
  canvas.beginShape();
...
void canvasSphere(PGraphics canvas, float row, float column, float radius){
  canvas.beginShape();
