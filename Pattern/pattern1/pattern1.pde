...
void setup() {
  ...
  mainShader = canvas.loadShader(
    "./shader/texture/Pattern/Pattern2Geometry.frag", 
    "./shader/texture/Pattern/Pattern2Geometry.vert"
  );
}
...
void createPolygon() {
  ...
  float polygonCount = abs(sin(uTime * 0.5) * 10.0) + 2.0;
  ...
  canvasPolygon(canvas, canvas.width * 0.5, polygonCount, polygonCount);
  canvas.popMatrix();
  canvas.endDraw();
