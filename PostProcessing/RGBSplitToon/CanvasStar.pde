class CanvasStar implements StarType {
  ...
  void drawShaderStar(PShader mainShader) {
    this.canvas.shader(mainShader);
    float vertexNumber = pricleNumber * 2.0;
    float segemt = segemtNumber * 2.0;
    this.canvas.beginShape();
    for (float theta = 0; theta <= segemt; theta++) {
  ...
  void drawShaderTextureStar(PShader mainShader, PImage textureImage) {
    this.canvas.shader(mainShader);
    mainShader.set("uTexture", textureImage);
    float vertexNumber = pricleNumber * 2.0;
    float segemt = segemtNumber * 2.0;
    this.canvas.beginShape();
    for (float theta = 0; theta <= segemt; theta++) {
