...
vec4 calcColor(vec2 coord){
  ...
  vec4 calcTexture3 = singlePassBlur(coord, uBackground);
  vec4 calcTexture = calcTexture1 *  rgbDisplacementMap(coord, calcTexture2);
  // vec4 calcTexture = rgbDisplacementMap(coord, calcTexture1) *  rgbDisplacementMap(coord, calcTexture2);
