...
vec3 createSea(vec2 coord, vec4 texture, vec2 mouse){
  // Base Sea and Water
  vec3 seaColor1 =vec3(0.0392, mouse.x, mouse.y);
  ...
  vec3 textureColor = vec3(texture.r, texture.g, texture.b);

  vec3 color = mix(seaColor, textureColor, ring(coord, radius));
...
void main(){
  ...
  vec3 color = createSea(coord, texture, mouse);
  // vec3 color = vec3(texture.r, texture.g, texture.b);
