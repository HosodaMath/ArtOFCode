...
float flower(vec2 coord, float flowerPetal){
  ...
  for(int x = 0; x < iter; x++){
    float y = float(x) + 0.1;
    float s = cos(uTime * fbm(vec3(coord * 10.0, abs(cos(uTime * 0.5)))) * y);
    float c = sin(uTime * fbm(vec3(coord * 10.0, abs(cos(uTime * 0.5)))) * y);
    vec2 q = coord + vec2(s, c) * 0.5;
    float u = sin((atan(q.x, q.y) + uTime * 0.5) * flowerPetal);
    dest += 0.01 / abs(u - length(coord));
  }
...
vec3 create(vec2 position){
  ...
  vec3 flowerColor1 = vec3(0.18, 0.37, 1.0);
  vec3 flowerColor2 = vec3(0.502, 1.0, 1.0);
  float flowerPetal = 6.0;
  vec3 flowerColor = mix(flowerColor1, flowerColor2, coord.y);

  vec3 warter_color = createWarter(position);
  vec3 main_color = createMainColor(coord);
  
  vec3 calcWaterColor = warter_color * main_color;
  //vec3 calcColor = mix(calcWaterColor, ringColor, ring(coord, 0.2));
  vec3 calcColor = mix(calcWaterColor, flowerColor, flower(coord, flowerPetal));
