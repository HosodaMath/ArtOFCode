...
float ring(vec2 coord ,float radius){
  ...
  for(int x = 0; x < iter; x++){
    float y = float(x) + 0.1;
    float s = cos(uTime * fbm(vec3(coord * 10.0, abs(cos(uTime * y)))) * 0.5);
    float c = sin(uTime * fbm(vec3(coord * 10.0, abs(sin(uTime * y)))) * 0.5);
...
vec3 create(vec2 position){
  ...
  vec3 ringColor1 = vec3(0.18, 0.37, 1.0);
  vec3 ringColor2 = vec3(0.502, 1.0, 1.0);
  vec3 ringColor = mix(ringColor1, ringColor2, coord.y);
  float calcRing = ring(coord, 0.5);
  ...
  vec3 calcColor = mix(calcWaterColor, ringColor, calcRing);
