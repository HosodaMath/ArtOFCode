...
void main(){
  ...
  vec4 colorData1 = vec4(0.0392, 0.0314, 0.4745, 1.0);
  vec4 colorData2 = vec4(0.0, 0.0039, 0.2314, 1.0);
  vec4 seaColor = mix(colorData1, colorData2, coord.y);

  vec2 coord2 = -1.0 + 2.0 * gl_FragCoord.xy / uResolution.xy;
  coord2 = waveEffect(coord2);
  ...
  /*
  float intensity = max(0.0, dot(vNormal, vLightDirection));
  vec4 shadowColor;
  if(intensity > 0.95){
    shadowColor = vec4(0.7804, 0.7843, 1.0, 1.0);
    bgColor += shadowColor;
  } else if(intensity > 0.5){
    shadowColor = vec4(0.4745, 0.5059, 0.9216, 1.0);
    bgColor += shadowColor;
